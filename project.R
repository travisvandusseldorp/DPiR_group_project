...
# Group by specific conference (ACC) and position, return with total players drafted by year
Overall_ACC <- draft_results %>%
  group_by(Conference = "ACC", Team, Year,) %>%
  summarize(Players_Drafted = n())
...
# Creating a standard theme for the project to avoid reformating each graph
...
ggplot(data = number_players_drafted, 
       aes_(x=number_players_drafted$Position, 
            y=number_players_drafted$Players_Drafted, 
            group=number_players_drafted$Conference)) +
...
c <- c + labs(title = "Number of Players Drafted by Year") + 
         theme_project()
c 
...
d <- ggplot(data = ACC_Year,
            aes_(x=ACC_Year$Year,
                 y=ACC_Year$Players_Drafted,
                 group=ACC_Year$Position)) +
            geom_line(aes(color=ACC_Year$Position)) +
            geom_point(aes(color=ACC_Year$Position))

d <- d + theme_minimal() +
         labs(color = "Position") +
         scale_y_continuous(name="Players Drafted", limits=c(0, 60))
d
...
# Try this line and see if you like the look better, I think horrizontal looks better if we put one graph on a slide but vertical if two
...
p1 <- acc %>% 
  count(College) %>%
  mutate(College = fct_reorder(College, n, .desc = TRUE)) %>%
  top_n(5) %>% 
  ggplot(aes(x = College, y = n)) +
  geom_bar(stat = 'identity', 
           aes(fill = College), show.legend = FALSE) +
  labs(title = "Colleges Most Drafted in ACC", x = "", y = "") +
  theme_project()
...
p2 <- acc %>% 
    count(Team) %>%
    mutate(Team = fct_reorder(Team, n, .desc = TRUE)) %>% 
    top_n(5) %>% 
    ggplot(aes(x = Team, y = n)) + 
    geom_bar(stat = 'identity', 
             aes(fill = Team), show.legend = FALSE) +
    labs(title = "Teams Most Likely to Draft an ACC Player", x = "", y = "") +
    theme_project()
